...
- `cargo test -p mindustry-desktop player_list_team_select --lib -- --nocapture`：通过，`2 passed; 0 failed`。
- `cargo test -p mindustry-desktop player_list_votekick --lib -- --nocapture`：通过，`2 passed; 0 failed`。
- `cargo test -p mindustry-desktop trace_dialog --lib -- --nocapture`：通过，`4 passed; 0 failed`。
- `cargo test -p mindustry-desktop admin_list --lib -- --nocapture`：通过，`2 passed; 0 failed`。
- `cargo test -p mindustry-desktop player_list_ --lib -- --nocapture`：通过，`18 passed; 0 failed`。
- `cargo test -p mindustry-core trace_dialog --lib -- --nocapture`：通过，`1 passed; 0 failed`。
- `cargo test -p mindustry-core unban_player_id --lib -- --nocapture`：通过，`1 passed; 0 failed`。
- Workspace crate：
  - `mindustry-core`
  - `mindustry-server`
  - `mindustry-desktop`
...
- `desktop/src/lib.rs` 已补 `TraceDialog` 桌面端前端闭环：新增独立 `Ui` 模态 pass，按上游 `BaseDialog("@trace") + addCloseButton()`、`Table(Tex.clear).margin(14)`、4 个 `Styles.emptyi`/`Icon.copySmall` 28x28 复制按钮、summary 行、`@trace.ips/@trace.names` 列表和 `@back + Icon.left` 绘制；新增 `player_trace_dialog_action_at_surface_point` 与 `DesktopPlayerTraceDialogAction`，接入鼠标点击、背景点击吞掉、Escape/back 关闭和 `graphics_frame_for_render`；新增 `desktop_launcher_trace_dialog_renders_and_clicks_like_java` 覆盖 pass、文本、按钮命中、剪贴板复制反馈和关闭行为。
...
- `BansDialog` / `AdminsDialog` 桌面前端闭环：`core/src/mindustry/net/administration.rs` 补 `unban_player_id()`，按上游 `unbanPlayerID` 清除 id ban 并移除该玩家用过的 IP ban；`desktop/src/lib.rs` 将 `BansDialogModel` / `AdminsDialogModel` 渲染为独立 `Ui` 模态 pass，接入 `graphics_frame_for_render`、hover/click、滚轮可见行偏移、Back/Escape、Enter、`@back`、行内 `Icon.cancel` 与二级 `@confirm` 弹窗。确认 unban 后刷新封禁列表，确认 unadmin 后同步 `Administration::unadmin_player` 并把同 uuid 的远端 `PlayerComp.admin` 置 false；背景点击只消费不穿透，client 仍不能打开服务端管理列表。新增 `desktop_launcher_admin_list_bans_dialog_renders_scrolls_and_unbans_like_java`、`desktop_launcher_admin_list_admins_dialog_confirms_and_syncs_remote_like_java` 与 `unban_player_id_clears_id_and_used_ips_like_java`，并复跑 `admin_list`、`player_list_`、`unban_player_id`、workspace check 与 `git diff --check` 通过。
...
- `PlayerListFragment` 客户端 `votekick` 文本输入链：`desktop/src/lib.rs` 新增 `DesktopPlayerListVoteKickInput` 与 `dispatch_player_list_row_action_like_java()`，客户端同队非管理员行的 `StartVoteKick` 现在打开上游 `ui.showTextInput("@votekick.reason", Core.bundle.format("votekick.reason.message", user.name()), "", ...)` 等价状态，保留默认 32 字符、非数字、非空输入约束；确认原因后不直接发 player-list 特例 packet，而是复用聊天动作链生成 `ChatAction::FireClientChatEvent` 与 `ChatAction::SendChatMessage("/votekick #<id> <reason>")`，取消不发送。新增 `desktop_launcher_player_list_votekick_input_sends_chat_command_like_java`，并复跑 votekick/core player-list 定向测试通过。
...
- `PlayerListFragment` `@player.team` 二级队伍选择弹窗前端闭环：`desktop/src/lib.rs` 新增 `DesktopPlayerListTeamSelectDialogAction`、`player_list_team_select_render_pass()` 与 `player_list_team_select_action_at_surface_point()`，把 `teamSelect.setFillParent(false)` 渲染为独立 `Ui` 模态；布局按上游 `BaseDialog(Core.bundle.get("player.team") + ": " + user.name)`、`Tex.whiteui + Styles.clearNoneTogglei`、50px 按钮、4px image margin、三列换行、当前队伍 checked、team color tint 和 `addCloseButton()` 的 210x64 `@back + Icon.left` 还原；primary click 命中队伍后发送 `AdminRequestCallPacket(AdminAction::SwitchTeam, TypeValue::Team(team_id))` 并关闭，背景点击只消费，Back/Escape 关闭且不发包，关闭 PlayerList 时同步清理队伍弹窗。新增 `desktop_launcher_player_list_team_select_dialog_renders_and_clicks_like_java`，覆盖 render pass、graphics frame 接入、按钮色块、hit-test、背景消费、关闭按钮、Escape 与队伍点击分发，并复跑 PlayerList/core/workspace 检查通过。
- `PlayerListFragment` 客户端 `votekick` `UI.showTextInput` 前端闭环：`desktop/src/lib.rs` 扩展 `DesktopPlayerListVoteKickInput` 的当前输入文本，新增 `DesktopPlayerListVoteKickInputDialogAction`、`player_list_votekick_input_render_pass()` 与 `player_list_votekick_input_action_at_surface_point()`，把上游 `ui.showTextInput("@votekick.reason", Core.bundle.format("votekick.reason.message", user.name()), "", ...)` 桌面分支渲染为独立 `Ui` 模态；布局按 Java `Dialog(titleText)`、`cont.margin(30)`、消息文本、330x50 `defaultField`、120x54 `@cancel/@ok` 按钮还原，空输入时禁用 OK；Text/Backspace/Delete 更新输入框，Enter 在非空时发送 `/votekick #<id> <reason>`，Escape/Back 与 Cancel 取消且不发包，背景点击只消费，关闭 PlayerList 时同步清理输入弹窗。新增 `desktop_launcher_player_list_votekick_input_dialog_renders_and_inputs_like_java`，覆盖 render pass、graphics frame 接入、TextField/cursor、OK 禁用、文本输入、退格、背景消费、OK 点击发送与 Escape 取消，并复跑 PlayerList/core/workspace 检查通过。
