...
- Current state:
  ...
  - M6 additive regression hardening update (2026-04-09): `effect_id=12` (`move_command`) now stays payload-driven for business projection instead of being forced through the generic `PositionTarget` contract path. Fresh local verification stayed green with `cargo test --manifest-path .\rust\mdt-client-min\Cargo.toml effect_packet_with_` (`42 passed`) and a full `cargo test --manifest-path .\rust\mdt-client-min\Cargo.toml` rerun (`1101` lib + `207` online bin + `2` replay-bridge tests passed, `0 failed`).
  ...
  - M7 additive runtime-seed surface update: `mdt-world` now also exposes bounded query surfaces around post-load runtime readiness, including `runtime_seed_surface()` plus source-region readiness summaries. These surfaces expose `can_seed_runtime_apply/world_shell_ready`, region/batch summaries, and per-source blocked/awaiting/deferred state as consumable read-only data, but direct live-runtime seeding remains open.
  - M7 validation refresh (2026-04-09): `cargo test --manifest-path .\rust\mdt-world\Cargo.toml` stayed green in a fresh isolated target/temp run (`431` lib + `4` main tests passed, `0 failed`). This is verification for the current bounded compatibility slice, not a claim of full Java live-world parity.
  - M7/M9 refactor hardening update (2026-04-26): save-post-load bool/source-region helper layers are now centralized (`lib.rs` shared `yes/no` and `1/0` labels plus `save_post_load_runtime_source_region.rs` shared step/source-region mapping + sort order), `ClientSessionEvent` now centralizes runtime transient reset trigger/reason classification for `WorldDataBegin` / `WorldStreamStarted` / `ConnectRedirectRequested`, and runtime UI compact-text / Unicode-length / URI-scheme helpers now flow through `presenter_view.rs` as a single source for ASCII/window/HUD consumers. Fresh local verification stayed green with `powershell -ExecutionPolicy Bypass -File .\tools\verify-rust-workspaces.ps1`, `cargo test --manifest-path .\rust\mdt-world\Cargo.toml source_region_name` (`2 passed`), `cargo test --manifest-path .\rust\mdt-render-ui\Cargo.toml runtime_ui_` (`30 passed`), and `cargo test --manifest-path .\rust\mdt-client-min\Cargo.toml preferred_runtime_transient_reset_reason_prioritizes_redirect_then_stream_then_begin` (`1 passed`). This reduces internal divergence; it does not widen the external parity claim.
  ...
  - M8 additive regression hardening update (2026-04-09): command-mode `SetTarget` now preserves existing `command_buildings` while updating the target projection, so `SelectBuilding(Some(...)) -> SetTarget -> SetCommand/SetStance` no longer regresses back to unit-style selection clearing. Fresh local verification stayed green with `cargo test --manifest-path .\rust\mdt-client-min\Cargo.toml --bin mdt-client-min-online` (`207 passed`) and the same full `mdt-client-min` rerun noted above. This remains bounded minimal-compatibility behavior, not a full Java desktop parity claim.
  ...
  - M8 additive batch/override consistency update: `mdt-input` batch mapping now also respects runtime override state through `map_snapshot_batch_or_override(...)`, and `RuntimeIntentTracker::sample_runtime_snapshot_batch()` now clears stale `pressed/released` edges on empty batches while preserving ongoing active/build/mining state. This closes a bounded batch/override consistency gap without claiming broader desktop/mobile live-input parity.
  ...
  - M8 additive builder-stage update: `BuilderQueueHeadExecutionObservation::ActiveConstruct` now promotes the current head into `InFlight` and refreshes queued/in-flight counters instead of leaving the local queue head in a stale queued-only state. This is still a bounded builder-queue semantic slice, not full Java build execution parity.
  ...
  - M9 additive icon/overlay update: the same primitive/model expansion now also includes `RenderPrimitive::Icon` for bounded high-signal runtime overlay families, including `runtime-effect-icon`, `runtime-build-config-icon`, `runtime-health`, `runtime-place`, the command-mode marker families (`runtime-command-building`, `runtime-command-selected-unit`, `runtime-command-build-target`, `runtime-command-position-target`, `runtime-command-unit-target`), the runtime unit-assembler families, and the current low-risk world-event families (`runtime-break`, `runtime-bullet`, `runtime-logic-explosion`, `runtime-sound-at`, bounded tile-action markers such as `runtime-auto-door-toggle`). Command-mode world feedback now also emits bounded generic target-link lines from selected units and command buildings toward the resolved target position instead of stopping at icon-only endpoints. ASCII/window presenters now consume those typed icon primitives directly, emit `RENDER-ICON` summaries, and no longer rely on generic point fallback for those families. This is still presenter-local observability shaping, not layered renderer or Java desktop UI parity closure.
  - M9 additive minimap ping update: inbound `pingLocation` now also surfaces as a bounded runtime scene text marker (`marker:text:runtime-ping:*`) and the window minimap inset projects its latest tile as a dedicated ping point, while `worldDataBegin` clears the cached ping projection so a newly loading world does not inherit stale ping markers.
  - M9 additive minimap world-label update: the window minimap inset now also projects a bounded deduped set of current `world-label:*` scene-object tiles, so existing runtime/live world labels surface as low-risk minimap points without adding a new protocol or renderer backend path.
  - M9 additive minimap runtime-overlay update: the window minimap inset now also carries a bounded priority-sorted runtime overlay point set for the low-frequency `runtime-config*`, `runtime-break`, and `runtime-place` families, deduped by tile and drawn beneath player/focus/ping markers so configuration/build-plan hotspots can surface without flooding the minimap with higher-frequency combat/runtime events.
  - M9 additive minimap command update: the window minimap inset now also projects a bounded priority-sorted `runtime-command` point set from the existing command-mode object ids (`build-target`, `position-target`, `unit-target`, `selected-unit`, `building`), deduped by tile and drawn beneath player/focus/ping markers so command intent can surface without needing deeper command rect/line minimap reconstruction yet.
  - M9 additive minimap command-rect update: the window minimap inset now also consumes the existing `RenderPrimitive::Rect` projection for `runtime-command-rect` and `runtime-command-target-rect`, reusing the already derived rect primitives instead of rebuilding line-end pairs inside the presenter, so command selection/target boxes surface as bounded minimap outlines.
  - M9 additive minimap tile-action update: the window minimap inset now also projects a bounded low-frequency tile-action point set from the existing spawn-like runtime event ids (`runtime-unit-block-spawn`, `runtime-unit-tether-block-spawned`, `runtime-landing-pad-landed`, `runtime-assembler-drone-spawned`, `runtime-assembler-unit-spawned`), while intentionally excluding noisier higher-frequency families such as `runtime-auto-door-toggle`.
  - M9 additive minimap break-rect update: the window minimap inset now also consumes the existing `RenderPrimitive::Rect` projection for `runtime-break-rect`, projecting bounded deconstruct-area outlines onto the minimap so break-plan coverage is visible as area feedback instead of only the existing point markers.
  - M9 additive minimap unit-assembler update: the window minimap inset now also projects a bounded point set from the existing `runtime-unit-assembler-progress` and `runtime-unit-assembler-command` marker families, deduped by tile so assembler activity and target state remain visible on the minimap without widening runtime/session scope.
  - M9 additive minimap unit-assembler-area update: `mdt-render-ui` now also derives dedicated `RenderPrimitive::Rect` entries from the existing `runtime-unit-assembler-area:{block}:{tile_x}:{tile_y}:{edge}` marker lines instead of collapsing all same-family areas into one rect key, and the window minimap inset now projects bounded assembler-footprint outlines from those rect primitives so assembler area coverage is visible alongside the existing assembler point markers.
  - M9 additive minimap deconstruct update: the window minimap inset now also treats `terrain:runtime-deconstruct:*` as part of the existing bounded break/deconstruct overlay channel, so recent deconstruct authority feedback surfaces on the minimap instead of remaining only in detail counters and main-scene terrain markers.
...
- `mdt-input` runtime batch sampling now also respects override state and clears stale edge-only residue on empty batches.
  - `map_snapshot_batch_or_override(...)` is now the shared bounded path for batch-vs-override consistency, while active/build/mining state remains preserved across empty-batch sampling.
  - this is a batch/override consistency fix, not a claim of full live-input parity.
...
- `mdt-world` now also exposes a richer post-load runtime readiness query surface.
  - `runtime_seed_surface()` and source-region readiness helpers expose bounded readiness/blocked/awaiting/deferred summaries for consumers without forcing tests or callers to re-walk internal readiness state.
  - this is still a read-only/runtime-preflight surface; it does not mean direct live-runtime seeding is complete.
...
- `P0` unit semantic retention in `rust/mdt-client-min/src/session_state.rs` + `rust/mdt-client-min/src/client_session.rs`
  - Current gap: bounded runtime-sync widening is landed, but only for the current low-risk slice. Rust now retains `ammo_bits/elevation_bits/flag_bits`, mech `base_rotation_bits`, and unit carried-item stack mirrors on the typed runtime sidecar, while broader family breadth plus deeper `status/payload/controller` semantics remain open.
...
- `mdt-render-ui` now also exposes a bounded text primitive channel.
  - Landed shape:
