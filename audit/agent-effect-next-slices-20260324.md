...
1. `263` `legDestroy`
2. `9` / `8` `itemTransfer` / `unitSpirit`

Why this order:

- `252`, `26`, `256`, and now the first `257/260` parent-shield fallback geometry slice are already landed as narrow executor-focused slices, so the clearest remaining gap shifts to `legDestroy` and the cheap source-target movers.
- `257/260` no longer need a first geometry pass; their remaining work is narrower metadata parity, not generic marker parity.
