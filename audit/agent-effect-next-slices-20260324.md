...
- `8` `unitSpirit` is the cheapest remaining executor-only slice because it can stay on the existing `position_target` contract and only add effect-specific double-diamond line geometry over the already captured source/target bits.
- `9` `itemTransfer` is still narrow, but its Java look depends on a stable effect-instance seeded lateral offset; that makes it a better follow-up once Rust carries a deterministic per-overlay seed instead of faking it ad hoc.
