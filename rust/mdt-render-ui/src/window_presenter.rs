...
fn compose_frame_panel_lines(
    ...
    if let Some(minimap_flow_detail_text) =
        build_minimap_user_flow_panel(scene, hud, window).map(|panel| panel.detail_label())
    {
    ...
    if let Some(build_interaction_detail_text) =
...
fn compose_render_text_detail_status_text(
    ...
    let total = text_primitives.len();
    let detail_items = text_primitives
        .into_iter()
        .map(|(kind_label, layer, tile_x, tile_y, payload)| {
            format!(
            "{} payload[{}]",
            format_render_text_signature(kind_label, layer, tile_x, tile_y),
            format_render_primitive_payload_fields(&payload)
        )
        });
    Some(format_counted_detail_text(total, " ", detail_items))
...
fn compose_render_rect_status_text(
    ...
    let preview_items = rect_primitives
        .into_iter()
        .take(2)
        .map(|(family, layer, left, top, right, bottom)| {
...
fn compose_render_icon_status_text(
    ...
    let preview_items = icon_primitives
        .into_iter()
        .take(2)
        .filter_map(|(family, variant, layer, x, y)| {
            let (tile_x, tile_y) = finite_tile_coords(x, y)?;
            Some(format_render_icon_signature(
            family.label(),
            &variant,
            layer,
            tile_x,
...
fn compose_render_icon_detail_status_text(
    ...
    let detail_items = icon_primitives.into_iter().map(
        |(family, variant, layer, tile_x, tile_y, payload)| {
            format!(
            "{} payload[{}]",
...
fn compose_build_ui_status_text(build_ui: &BuildUiObservability) -> String {
    format!(
        ...
        format_optional_i16_text(build_ui.selected_block_id),
...
mod tests {
    ...
    #[test]
    fn window_build_interaction_queue_inflight_uses_flight_token() {
        assert_eq!(
            super::build_interaction_queue_status_text(
    ...
    fn present_once_appends_structured_hud_slices_to_frame_status_text() {
        ...
        assert_frame_line_contains(
            &frame.panel_lines,
            "RUNTIME-STACK-DEPTH: sdepth:p2:n4:c1:m2:h4:d7:g3:t7",
        );
        assert_frame_line_contains(
            &frame.panel_lines,
    ...
    fn present_once_surfaces_build_config_overflow_and_extended_minimap_counts() {
        ...
        assert_frame_line_contains(
            &frame.panel_lines,
            "BUILD-MINIMAP-AUX: bmaux:m=place:s=head-aligned:q=mixed:r1:c=3/7@gamma:a=rej-miss-build:p=match:h=10:12:t=10:12:x=tilecfg:b=gamma:f=0:0@1:v0:u100:w0:d75:o3:rt0:rs0",
        );
        assert_frame_line_contains(
            &frame.panel_lines,
