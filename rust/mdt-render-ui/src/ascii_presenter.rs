...
use crate::presenter_view::{
    ...
    format_runtime_reconnect_detail_text_if_nonempty, format_runtime_reconnect_row_text,
    ...
    compose_runtime_stack_depth_text_from_hud, compose_runtime_stack_detail_text_from_hud,
    compose_runtime_stack_panel_text_from_hud,
...
impl AsciiScenePresenter {
    ...
    fn compose_frame(&self, scene: &RenderModel, hud: &HudModel) -> String {
        ...
        if let Some(minimap_visibility_detail_text) = build_minimap_panel(scene, hud, window)
            .map(|minimap| format_minimap_visibility_detail_text(&minimap))
        {
            out.push_str(&format!(
        ...
        if let Some(minimap_kinds_text) = build_minimap_panel(
            scene,
            hud,
            PresenterViewWindow {
                origin_x: 0,
        ...
        if let Some(minimap_kinds_detail_text) = build_minimap_panel(
            scene,
            hud,
            PresenterViewWindow {
                origin_x: 0,
        ...
        .map(|panel| compose_minimap_window_kind_distribution_text(&panel))
        {
            out.push_str(&format!(
                "MINIWIN-KINDS: {minimap_window_kinds_text}\n"
            ));
        }
        if let Some(minimap_window_text) = build_minimap_panel(
            scene,
            hud,
            PresenterViewWindow {
                origin_x: 0,
        ...
        .map(|panel| compose_minimap_window_distribution_text(&panel))
        {
        ...
        if let Some(minimap_edge_text) = build_minimap_panel(scene, hud, window)
            .map(|panel| compose_minimap_edge_summary_text(&panel))
        {
        ...
        if let Some(minimap_edge_detail_text) = build_minimap_panel(scene, hud, window)
            .map(|panel| format_minimap_edge_detail_text(&panel))
        {
            out.push_str(&format!(
                "MINIMAP-EDGE-DETAIL: {minimap_edge_detail_text}\n"
            ));
        }
        if let Some(panel) = build_minimap_panel(scene, hud, window) {
            for minimap_detail_text in format_minimap_detail_lines(&panel) {
        ...
        if let Some(render_line_text) = compose_render_line_status_text(scene, window) {
            out.push_str(&format!("RENDER-LINE: {render_line_text}\n"));
        }
        if let Some(render_line_detail_text) = compose_render_line_detail_text(scene, window) {
            out.push_str(&format!("RENDER-LINE-DETAIL: {render_line_detail_text}\n"));
        }
    ...
    let mut line_primitives = scene
        ...
        .filter_map(|primitive| match &primitive {
            RenderPrimitive::Line {
                ...
                let (start_tile_x, start_tile_y) = finite_world_tile(*x0, *y0)?;
                let (end_tile_x, end_tile_y) = finite_world_tile(*x1, *y1)?;
...
fn compose_build_config_authority_summary_text(
    ...
    format!(
        ...
        build_config_rollback_source_text(authority_source),
        compact_runtime_ui_text(authority_block_name),
    )
}
...
mod tests {
    ...
    #[test]
    fn ascii_presenter_build_config_rollback_source_text_maps_all_states() {
        assert_eq!(super::build_config_rollback_source_text(None), "none");
        assert_eq!(
            super::build_config_rollback_source_text(Some(
                crate::BuildConfigAuthoritySourceObservability::TileConfig,
            )),
            "tileConfig"
        );
    ...
    fn build_config_outcome_compact_text_maps_all_variants_stably() {
        ...
        assert_eq!(super::build_config_outcome_compact_text(None), "none");
    }
    ...
    fn ascii_presenter_keeps_reconnect_visible_when_loading_is_active() {
        ...
        assert!(presenter.last_frame().contains(
            "SESSION-BANNER: RELOAD @lw1:cl0:rd1:cc0:p4:d5:r6 | RECONNECT attempt3:redirect@1/127.0.0.1:6567:connectRedir~@none:server_reque~ | LOADING defer5:replay6:drop7:qdrop8:sfail0:scfail0:efail0:rdy12@1300:to2:cto1:rto1:ltready@20000:rs3:rr1:wr1:kr1:lrreload:lwr@lw1:cl0:rd1:cc0:p4:d5:r6"
        ));
    }
    ...
    fn ascii_presenter_reports_runtime_world_reload_summary() {
        ...
        let frame = presenter.last_frame();
        assert!(frame.contains(
            "SESSION-BANNER: RELOAD @lw1:cl0:rd1:cc0:p4:d5:r6 | LOADING defer5:replay6:drop7:qdrop8:sfail0:scfail0:efail0:rdy12@1300:to2:cto1:rto1:ltready@20000:rs3:rr1:wr1:kr1:lrreload:lwr@lw1:cl0:rd1:cc0:p4:d5:r6"
        ));
    ...
    fn ascii_presenter_surfaces_minimap_and_build_config_overflow_context() {
        ...
        let build_minimap_aux_pos = frame
            .lines()
            .position(|line| {
                line == "BUILD-MINIMAP-AUX: bmaux:m=place:s=head-aligned:q=mixed:r1:c=4/8@gamma:a=rej-miss-build:p=match:h=10:12:t=10:12:x=tilecfg:b=alpha:f=1:1@1:v0:u100:w0:d75:o3:rt0:rs0"
            })
        ...
        assert!(frame.contains(
            "BUILD-MINIMAP-AUX: bmaux:m=place:s=head-aligned:q=mixed:r1:c=4/8@gamma:a=rej-miss-build:p=match:h=10:12:t=10:12:x=tilecfg:b=alpha:f=1:1@1:v0:u100:w0:d75:o3:rt0:rs0"
        ));
        assert!(frame.contains(
    ...
    #[test]
    fn ascii_presenter_reports_runtime_notice_state_detail() {
        let scene = runtime_stack_test_scene();
        let mut presenter = AsciiScenePresenter::default();
        let mut runtime_ui = RuntimeUiObservability::default();
        runtime_ui.hud_text.set_count = 1;
    ...
    #[test]
    fn ascii_presenter_surfaces_text_primitive_summary() {
        let scene = RenderModel {
            viewport: Viewport {
                width: 32.0,
                height: 16.0,
    ...
    #[test]
    fn ascii_presenter_reports_render_line_overflow_count() {
        let scene = RenderModel {
            viewport: Viewport {
                width: 32.0,
                height: 24.0,
        ...
        let frame = presenter.last_frame();
        assert!(frame.contains("RENDER-LINE: count=3"));
        assert!(frame.contains("marker-line@30:0:0->1:0"));
        assert!(frame.contains("marker-line@31:0:1->1:1"));
        assert!(frame.contains("more=1"));
        assert!(frame.contains("RENDER-LINE-DETAIL: count=3"));
        assert!(frame.contains("marker-line@32:0:2->1:2 payload[marker_id=demo-c]"));
    }
