...
impl SessionState {
    ...
    pub fn record_block_snapshot_parse_failure(&mut self, error: String, payload_len: usize) {
        ...
        self.last_block_snapshot_parse_error = Some(error);
        self.last_block_snapshot_parse_error_payload_len = Some(payload_len);
    }

    pub fn record_state_snapshot_parse_failure(&mut self, error: String, payload_len: usize) {
        self.failed_state_snapshot_parse_count =
            self.failed_state_snapshot_parse_count.saturating_add(1);
        self.last_state_snapshot_parse_error = Some(error);
    ...
    pub fn clear_state_snapshot_core_data_parse_failure(&mut self) {
        self.last_state_snapshot_core_data_parse_error = None;
    ...
    pub fn record_set_hud_text_reliable(&mut self, message: &Option<String>) {
        self.received_set_hud_text_reliable_count =
            self.received_set_hud_text_reliable_count.saturating_add(1);
    ...
    pub fn record_camera_position(&mut self, x: f32, y: f32) {
        self.received_set_camera_position_count =
            self.received_set_camera_position_count.saturating_add(1);
        self.last_camera_x_bits = Some(x.to_bits());
    ...
    pub fn mark_payload_lifecycle_build_removed(&mut self, build_pos: i32) {
        self.payload_lifecycle_projection.remove_building(build_pos);
    }
...
impl PayloadLifecycleProjection {
    pub fn remove_building(&mut self, build_pos: i32) {
        self.by_carrier.remove(&UnitRefProjection {
            kind: 1,
            value: build_pos,
        });

        for entry in self.by_carrier.values_mut() {
            if entry.target_build == Some(build_pos) {
                entry.removed_target_build = true;
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn session_state_block_snapshot_parse_failure_helper_tracks_error_text_and_payload_len() {
        let mut state = SessionState::default();

        state.record_block_snapshot_parse_failure(
            "blockSnapshot trailing bytes".to_string(),
            19,
        );
    ...
    #[test]
    fn session_state_state_snapshot_parse_failure_helper_tracks_error_text_and_payload_len() {
        let mut state = SessionState::default();

        state.record_state_snapshot_parse_failure(
            "stateSnapshot trailing bytes".to_string(),
            21,
        );
    ...
    #[test]
    fn session_state_clear_state_snapshot_core_data_parse_failure_clears_last_error_fields() {
        let mut state = SessionState::default();
    ...
    #[test]
    fn record_set_hud_text_reliable_tracks_message() {
        let mut state = SessionState::default();
        let message = Some("hud-reliable".to_string());

        state.record_set_hud_text_reliable(&message);
    ...
    #[test]
    fn record_camera_position_tracks_view_center_bits() {
        let mut state = SessionState::default();

        state.record_camera_position(123.5, -10.25);
    ...
    #[test]
    fn record_info_toast_tracks_message_and_duration() {
        let mut state = SessionState::default();
        let message = Some("toast".to_string());

        state.record_info_toast(&message, 1.5);
    ...
    #[test]
    fn payload_lifecycle_remove_building_drops_block_carrier_and_marks_build_target_removed() {
        let mut state = SessionState::default();
        let build_pos = pack_point2(5, 5);
        state.payload_lifecycle_projection.by_carrier.insert(
            UnitRefProjection {
                kind: 1,
                value: build_pos,
        ...
        state.mark_payload_lifecycle_build_removed(build_pos);
